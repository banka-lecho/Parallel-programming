...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            if (!account.invokeOperation()) {
                val acAccount = AcquiredAccount(account.amount, op)
                if (accounts[index].compareAndSet(account, acAccount)) {
                    return acAccount
                }
                ind += 1
            }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            // done
            val (minAc, maxAc) = listOf(acquire(min(fromIndex, toIndex), this), acquire(max(fromIndex, toIndex), this))
            val (from, to) = if (fromIndex > toIndex) listOf(maxAc, minAc) else listOf(minAc, maxAc)
            ...
            listOf(max(fromIndex, toIndex), min(fromIndex, toIndex)).forEach { release(it, this) }
        }
    }
}